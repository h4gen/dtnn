...
def prepare_data(dbpath, partitions, splitdst):
    ...
    train_data = data.ASEDataProvider(
        ...
        kvp={'total_energy': (1,)}, data={'atomic_forces': (19,3)}, prefetch=False, shuffle=True
    )
    val_data = data.ASEDataProvider(
        os.path.join(splitdst, 'validation.db'),
        kvp={'total_energy': (1,)}, data={'atomic_forces': (19,3)}, prefetch=True, shuffle=False
    )
...
def main(args):
    ...
    partitions = {'train': 400000, 'validation': 2000,
                  'test_live': 2000, 'test': -1}
...
#    U0 = np.array(train_data.get_property('total_energy'))
#    F0 = np.array(train_data.get_property('atomic_forces'))
#    E = U0.reshape((-1, 1))
...
#            'Should ISO17 atom references be downloaded to that location? [y/N]')
#
#        success = False
#        if do_download == 'y':
